function xkcdplot(parameters) {
  ...
  var width = 600,
    ...
    arrowOffset = 6,
    magnitude = 0.003,
    xlabel = parameters.xlabel || "Time of Day",
    ylabel = parameters.ylabel || "Awesomeness",
    title = parameters.title || "The most important graph ever made",
    xlim,
    ylim;
  ...
  xkcd.draw = function () {
    ...
    var inc = 5;
    for (var color in parameters.legend) {
      el.append("text").attr("class", "title")
        .attr("text-anchor", "start")
        .attr("x", 0)
        .attr("y", inc)
        .style("stroke", color)
        .text(parameters.legend[color]);
      inc += 22;
    }
